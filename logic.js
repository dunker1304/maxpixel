$(document).ready(function(){
  ...
  Circle.prototype.isSplitable = function(){
    //console.log(this.node, "isSplitable")
  ...
  const makeCircles = function(selector, colorData){
    ...
    function onSplit(circle){
      ...
      if (percent >= nextPercent) {
        console.log(Math.round(nextPercent * 100), "Percent Clear");
        nextPercent += 0.05;
      }
    }
    
    
    /// Make sure SVG exist end is empty
    if (!vis) {
      // create SVG element
  ...
  /// main logic
  var colorData = loadImage()
  makeCircles('#dots', colorData)
})